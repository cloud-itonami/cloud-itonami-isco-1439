(ns services-management.actor-test
  (:require [clojure.test :refer [deftest is testing]]
            [services-management.actor :as actor]
...
(deftest interrupts-then-commits-on-human-approval
  (let [st (fresh-store)
        graph (actor/build-graph {:store st})
        ;; quality-failure clearance always escalates (governor invariant)
        request {:client-id "client-1" :op :clear-quality-failure :stake :high}
