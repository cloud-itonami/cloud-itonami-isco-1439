(ns services-management.actor
  "ServicesManagementActor — the ISCO-08 1439 independent services-
  management actor as a `langgraph.graph/state-graph` (per
  ADR-2607011000 / CLAUDE.md Actors section). One graph run = one
  services operation request (intake → advise → govern → decide →
  ...
  The unconditional invariant: the ServicesAdvisor can never directly
  commit a record or dispatch a robot action the
  ServicesManagementGovernor refuses — every commit-record! call is
  gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [services-management.advisor :as advisor]
            [services-management.governor :as governor]
            [services-management.store :as store]))

(defn build-graph
  "Build a compiled ServicesManagementActor graph. `store` implements
  `services-management.store/Store`. `advisor` implements
  `services-management.advisor/Advisor` (defaults to `mock-advisor`).
